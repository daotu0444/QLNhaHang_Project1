...
--Nhan vien
INSERT INTO NguoiDung(ID_ND,Email,MatKhau,Trangthai,Vaitro) VALUES (100,'NVMinhTu@gmail.com','123','Verified','Quan Ly');
...
ALTER SESSION SET NLS_DATE_FORMAT = 'dd-MM-YYYY';
--Co tai khoan
INSERT INTO NhanVien(ID_NV,TenNV,NgayVL,SDT,Chucvu,ID_ND,ID_NQL,Tinhtrang) VALUES (100,'Dao Minh Tu','10/05/2023','0848044725','Quan ly',100,100,'Dang lam viec');
...
CREATE OR REPLACE TRIGGER TG_SLHD_CTT
FOR INSERT OR UPDATE OF ID_KH, TrangThai ON HoaDon
COMPOUND TRIGGER

    TYPE t_kh_ids IS TABLE OF HoaDon.ID_KH%TYPE;
    v_kh_ids t_kh_ids := t_kh_ids();

    BEFORE EACH ROW IS
    BEGIN
        -- Chỉ kiểm tra khi thêm hoặc đổi trạng thái thành 'Chua thanh toan'
        IF :NEW.TrangThai = 'Chua thanh toan' THEN
            v_kh_ids.EXTEND;
            v_kh_ids(v_kh_ids.LAST) := :NEW.ID_KH;
        END IF;
    END BEFORE EACH ROW;

    AFTER STATEMENT IS
    BEGIN
        FOR i IN 1 .. v_kh_ids.COUNT LOOP
            DECLARE
                v_count NUMBER;
            BEGIN
                SELECT COUNT(*)
                INTO v_count
                FROM HoaDon
                WHERE ID_KH = v_kh_ids(i)
                  AND TrangThai = 'Chua thanh toan';

                IF v_count > 1 THEN
                    RAISE_APPLICATION_ERROR(-20000,
                        'Moi khach hang chi duoc co toi da mot hoa don co trang thai chua thanh toan');
                END IF;
            END;
        END LOOP;
    END AFTER STATEMENT;
