...
public class ServiceAdmin {
    ...
    String sql = 
        "WITH rev AS ( " +
        "    SELECT EXTRACT(DAY FROM NgayHD) AS Ngay, SUM(TongTien) AS Revenue " +
        "    FROM HoaDon " +
        "    WHERE EXTRACT(MONTH FROM NgayHD) = EXTRACT(MONTH FROM CURRENT_DATE) " +
        "    AND EXTRACT(YEAR FROM NgayHD) = EXTRACT(YEAR FROM CURRENT_DATE) " +
        "    GROUP BY EXTRACT(DAY FROM NgayHD) " +
        "), " +
        "cost AS ( " +
        "    SELECT EXTRACT(DAY FROM NgayNK) AS Ngay, SUM(TongTien) AS Cost " +
        "    FROM PhieuNK " +
        "    WHERE EXTRACT(MONTH FROM NgayNK) = EXTRACT(MONTH FROM CURRENT_DATE) " +
        "    AND EXTRACT(YEAR FROM NgayNK) = EXTRACT(YEAR FROM CURRENT_DATE) " +
        "    GROUP BY EXTRACT(DAY FROM NgayNK) " +
        ") " +
        "SELECT COALESCE(r.Ngay, c.Ngay) AS Ngay, " +
        "       COALESCE(r.Revenue, 0) AS Revenue, " +
        "       COALESCE(c.Cost, 0) AS Cost " +
        "FROM rev r FULL OUTER JOIN cost c ON r.Ngay = c.Ngay " +
        "ORDER BY Ngay";
    ...
    while (rs.next()) {
        ...
        ModelChart data = new ModelChart("Ngày " + day, new double[]{revenue, cost, profit});
        list.add(data);
    }
