    ...
    public void initCard(String filter) {
        try {
            ...
            if (filter.equals("Tháng này")) {
                ...
                // Doanh thu
if (pre_revenue == 0) {
    descR = "Không có dữ liệu tháng trước";
} else if (revenue > pre_revenue) {
    descR = "Tăng " + Math.round(((double)(revenue - pre_revenue) / pre_revenue) * 100) + "% so với tháng trước";
} else {
    descR = "Giảm " + Math.round(((double)(pre_revenue - revenue) / pre_revenue) * 100) + "% so với tháng trước";
}

// Chi phí
if (pre_expenses == 0) {
    descE = "Không có dữ liệu tháng trước";
} else if (expenses > pre_expenses) {
    descE = "Tăng " + Math.round(((double)(expenses - pre_expenses) / pre_expenses) * 100) + "% so với tháng trước";
} else {
    descE = "Giảm " + Math.round(((double)(pre_expenses - expenses) / pre_expenses) * 100) + "% so với tháng trước";
}

// Lợi nhuận
if (pre_profit == 0) {
    descP = "Không có dữ liệu tháng trước";
} else if (profit > pre_profit) {
    descP = "Tăng " + Math.round(((double)(profit - pre_profit) / pre_profit) * 100) + "% so với tháng trước";
} else {
    descP = "Giảm " + Math.round(((double)(pre_profit - profit) / pre_profit) * 100) + "% so với tháng trước";
}
