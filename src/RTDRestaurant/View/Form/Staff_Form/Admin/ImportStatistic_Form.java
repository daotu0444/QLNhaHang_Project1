    ...
    private void cmdExcelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdExcelActionPerformed
        ...
        try {
            ...
            for (int row = 0; row < model.getRowCount(); row++) {
             ...
             File folder = new File("D:\\ExportFile_Excel");
    if (!folder.exists()) folder.mkdirs();

    // ✅ Lưu file Excel vào thư mục tuyệt đối
    String filePath = "D:\\ExportFile_Excel\\DanhsachPNK_" + simpleDateFormat.format(new Date()) + ".xlsx";
    workbook.easy_WriteXLSXFile(filePath);

    // ✅ Lấy đường dẫn tuyệt đối và thông báo thành công
    File file = new File(filePath);
    String path = file.getAbsolutePath();
    System.out.println("✅ File Excel đã được lưu tại: " + path);
