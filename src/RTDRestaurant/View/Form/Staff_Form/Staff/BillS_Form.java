...
try {
    ...
    String tienKHText = txtTienKH.getText().trim();
    int tienKH = 0;
    if (!tienKHText.isEmpty()) {
        try {
            tienKH = Integer.parseInt(tienKHText.replaceAll("[^\\d]", ""));
    ...
    cs.newLine();
    cs.showText(df.format(tienKH) + "d"); // ✅ tránh NumberFormatException khi trống
    cs.newLine();
    ...
    // ✅ đảm bảo text kết thúc đúng, tránh warning "You did not call endText()"
    try { cs.endText(); } catch (Exception ignore) {}

    // ✅ Đóng ContentStream trước khi lưu file
    if (cs != null) cs.close();

    // ✅ Tạo thư mục lưu hóa đơn nếu chưa có
    File folder = new File("D:\\HoaDon");
    if (!folder.exists()) folder.mkdirs();

    // ✅ Lưu file vào thư mục tuyệt đối
    String filePath = "D:\\HoaDon\\HoaDon_ID-" + bill.getIdHoaDon() + ".pdf";
    invc.save(filePath);
    ...
    System.out.println("✅ Xuất hóa đơn thành công tại: " + path);

} catch (Exception ex) {
    ex.printStackTrace(); // hoặc logger
} finally {
    // ❌ Không cần đóng thêm cs/invc ở đây nữa – tránh đóng trùng
    // ✅ Đảm bảo luôn đóng cả hai kể cả khi lỗi
    try {
        if (cs != null) cs.close();
    } catch (Exception ignore) {}

    try {
        if (invc != null) invc.close();
    } catch (Exception ignore) {}
}
